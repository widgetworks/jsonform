...
export class Playground {
  ...
  // 
  loadWiwoExample(item){
    console.log('(playground.loadWiwoExample) item=', item);
    
    // Fetch both of these URLs and then re-init the editor
    Promise.all([
      fetch(item.schemaUrl, {
        mode: 'cors'
    ...
    ]).then(function(schemaAndJson){
      var schema = schemaAndJson[0];
      var json = schemaAndJson[1];
