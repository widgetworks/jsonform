...
import {IFormElementOrString} from "./types/IFormDescriptor";
...
export class FormTree {
    ...
    initialize(formDesc: IFormDescriptor) {
        ...
        // Normalise Bootstrap form classes
        var defaultClasses: IFormClasses = classes.getDefaultClasses(this.formDesc.isBootstrap2 || JsonForm.isBootstrap2);
        _.extend(defaultClasses, this.formDesc.defaultClasses);
    ...
    _convertSchemaV3ToV4(
        _schema: any,
        processedSchemaNodes: any[] = [],
        parentSchemaProxy: any = null,
        keys: string[] = []
    ){
        if (!_schema){
            // return schema;
