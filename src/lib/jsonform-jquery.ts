...
export type FormOptionsType = ('values' | 'submit' | 'validate') | IFormDescriptor;

$.fn.jsonForm = function(commandOrOptions: FormOptionsType, param1?) {
    var form: FormTree;

    if (commandOrOptions === 'values') {
        return getFormValue(this);
    }
    
    if (commandOrOptions === 'submit') {
        form = this.data('jsonform-tree');
        if (!form) return null;
        return form.submit();
    }
    
    if (commandOrOptions === 'validate') {
        form = this.data('jsonform-tree');
        if (!form) return null;
    ...
    let options: IFormDescriptor = _.defaults(
        {},
        commandOrOptions,
        {
            submitEvent: 'submit',
