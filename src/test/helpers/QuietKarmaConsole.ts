...
(function(){
    ...
    __karma__.__result_skipped_count = 0;

    __karma__.result = function(result){
        if (result.skipped) {
			// Check skipped flag first - because they are both skipped AND successful
			this.__result_skipped_count++;
			this.skipped++;
    ...
    __karma__.complete = function () {
		window.console.log(
			...
			'Skipped: '+this.__result_skipped_count+' tests'
		);
