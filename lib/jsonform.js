...
jsonform.fieldTemplate = function(inner) {
  return '<div class="control-group jsonform-error-<%= keydash %>' +
    ...
    '<%= (node.required && node.schemaElement && (node.schemaElement.type !== "boolean") ? " jsonform-required" : "") %>' +
...
formTree.prototype.buildFromLayout = function (formElement, context) {
  ...
  if (formElement.key) {
    ...
    function isRequiredField(schema) {
      var parts = formElement.key.split('.');
      var field = parts.pop();
      if (field.slice(-2) == '[]') return false;
      var parentKey = parts.join('.');
      // we need get parent schema's required value
      var requiredFields = parentKey ? getSchemaKey(schema.properties, parentKey).required : schema.required;
      var required = requiredFields && requiredFields.indexOf(field) >= 0;
      return required;
    }
    formElement.required = schemaElement.required === true || isRequiredField(this.formDesc.schema);
  ...
  if (options.validate!==false) {
    ...
    if (validator) {
      ...
      if (validator._vendor == 'jjv') {
        var v = validator.validate(this.formDesc.schema, values);
        if (v) {
          errors = [v];
        }
      }
