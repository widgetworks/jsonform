  ...
  var templ = {
    ...
    'onInsert': function(evt, node) {
      ...
      if (node.formElement && node.formElement.tagsinput) {
        var $input = $(node.el).find('input');
        if ($input.tagsinput) {
          $input.tagsinput(node.formElement.tagsinput);
        }
      }
...
jsonform.elementTypes = {
  ...
  'tagsinput':{
    'template':'<select name="<%= node.name %>[]" id="<%= id %>" data-role="tagsinput"' +
      ' class="<%= fieldHtmlClass || cls.textualInputClass %>" multiple' +
      '<%= (node.disabled? " disabled" : "")%>' +
      '<%= (node.required ? " required=\'required\'" : "") %>' +
      '> ' +
      '<% if (node.value) { _.each(node.value, function(key, val) { %><option value="<%= key %>"><%= key %></option> <% }); } %> ' +
      '</select>',
    'fieldtemplate': true,
    'inputfield': true,
    'onInsert': function(evt, node) {
