...
jsonform.fieldTemplate = function(inner) {
  return '<div class="<%= cls.groupClass %> jsonform-error-<%= keydash %>' +
...
var inputFieldTemplate = function (type, isTextualInput) {
  return {
...
jsonform.elementTypes = {
  ...
  'checkboxbuttons': {
    ...
    'onBeforeRender': function (data, node) {
      ...
      var choices = null;
      var choiceshtml = null;
      var template = '<label class="<%= cls.buttonClass %>">' +
  ...
  'tabarray': {
    'template': '<div id="<%= id %>"><div class="tabbable tabs-left">' +
      ...
      '</div>' +
      '<a href="#" class="<%= cls.buttonClass %> _jsonform-array-addmore"><i class="<%= cls.iconClassPrefix %>-plus-sign" title="Add new"></i></a> ' +
      '<a href="#" class="<%= cls.buttonClass %> _jsonform-array-deleteitem"><i class="<%= cls.iconClassPrefix %>-minus-sign" title="Delete item"></i></a></div>',
    'fieldtemplate': true,
    'array': true,
    'childTemplate': function (inner) {
    ...
    'onBeforeRender': function (data, node) {
      ...
      _.each(node.children, function (child, idx) {
        ...
        tabs += '<li data-idx="' + idx + '"' +
          ...
          escapeHTML(title) +
          ' <span href="#" class="_jsonform-array-item-delete"><i class="<%= cls.iconClassPrefix %>-remove" title="Remove item"></i></span>' +
          '</a></li>';
      });
      data.tabs = tabs;
    },
    'onInsert': function (evt, node) {
      ...
      $nodeid.find('> a._jsonform-array-addmore')
        .toggleClass('disabled', boundaries.maxItems >= 0 && node.children.length >= boundaries.maxItems);
      $nodeid.find('> a._jsonform-array-deleteitem')
        .toggleClass('disabled', boundaries.minItems >= 0 && node.children.length <= boundaries.minItems);
    }
  },
  'help':{
  ...
  'fieldset':{
    'template': '<fieldset class="<%= cls.groupClass %> jsonform-error-<%= keydash %> <% if (elt.expandable) { %>expandable<% } %> <%= elt.htmlClass?elt.htmlClass:"" %>" ' +
      '<% if (id) { %> id="<%= id %>"<% } %>' +
      '>' +
   ...
   */
  'question': {
    'template': '<div id="<%= node.id %>"><% _.each(node.options, function(key, val) { %><%= node.formElement.optionsType !== "radiobuttons" ? "<div class=\'radio\'>" : "" %><label class="<%= node.formElement.optionsType === "radiobuttons" ? cls.buttonClass : "" %><%= ((key instanceof Object && key.htmlClass) ? " " + key.htmlClass : "") %>"><input type="radio" <% if (node.formElement.optionsType === "radiobuttons") { %> style="position:absolute;left:-9999px;" <% } %>name="<%= node.id %>" value="<%= val %>"<%= (node.disabled? " disabled" : "")%>/><span><%= (key instanceof Object ? key.title : key) %></span></label><%= node.formElement.optionsType !== "radiobuttons" ? "</div>" : "" %> <% }); %></div>',
    'fieldtemplate': true,
    'onInsert': function (evt, node) {
      var activeClass = 'active';
      var elt = node.formElement || {};
...
formNode.prototype.generate = function () {
  var data = {
    ...
    cls: getDefaultClasses(this.ownerTree.formDesc.isBootstrap2 || jsonform.isBootstrap2),
    escape: escapeHTML
  };
  var template = null;
  var html = '';
  ...
  function resolveRefs(obj, defs)
  {
    Object.keys(obj).forEach(function(prop, index, array){
      ...
      if (def.$ref) {
        ...
        if (resolvedSchemaRefNodes.indexOf(def) < 0) {
...
formTree.prototype.buildFromLayout = function (formElement, context) {
  ...
  if (formElement.key) {
    ...
    // the schema
    if (!formElement.options && (schemaElement['enum'] || schemaElement.type === 'boolean')) {
      ...
      if (formElement.titleMap) {
        ...
        formElement.options = enumValues;
      }
    }
    else if (typeof formElement.options === 'object' && !Array.isArray(formElement.options)) {
